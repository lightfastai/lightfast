...
const transcribeTool = createTool({
	...
	inputSchema: z.object({
		audioData: z.string().describe("Base64 encoded audio data or audio file path"),
		language: z.string().default("en").describe("Language code for transcription (e.g., 'en' for English)"),
	}),
	outputSchema: z.object({
		success: z.boolean(),
...
const voiceSettingsTool = createTool({
	...
	inputSchema: z.object({
		...
		pitch: z.number().min(-2.0).max(2.0).default(0).describe("Voice pitch adjustment"),
		stability: z.number().min(0).max(1).default(0.5).describe("Voice stability (0 to 1)"),
		similarityBoost: z.number().min(0).max(1).default(0.5).describe("Voice similarity boost (0 to 1)"),
	}),
	outputSchema: z.object({
		success: z.boolean(),
		settings: z.object({
