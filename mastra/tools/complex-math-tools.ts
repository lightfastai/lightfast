  ...
  execute: async ({ context }) => {
    ...
    switch (operation) {
      ...
      case "multiply":
        ...
        const product = Array(matrixA.length).fill(null).map(() => Array(matrixB[0].length).fill(0));
...
export const derivativeTool = createTool({
  ...
  execute: async ({ context }) => {
    ...
    if (atPoint !== undefined) {
      const value = derivativeCoeffs.reduce((sum, coeff, index) => 
        sum + coeff * Math.pow(atPoint, index), 0
      );
