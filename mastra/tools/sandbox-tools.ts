...
export const executeSandboxCommandTool = createTool({
	...
	execute: async ({ context }) => {
		const { sandboxId, command, args, cwd, background } = context;
...
// Tool to create a sandbox with exposed ports
export const createSandboxWithPortsTool = createTool({
	id: "create_sandbox_with_ports",
	...
	execute: async ({ context }) => {
		const { runtime, ports, source } = context;
...
// Tool to get domain URL for a specific port from sandbox
export const getSandboxDomainTool = createTool({
	id: "get_sandbox_domain",
...
// Tool to list all exposed ports and their URLs for a sandbox
export const listSandboxRoutesTool = createTool({
	id: "list_sandbox_routes",
