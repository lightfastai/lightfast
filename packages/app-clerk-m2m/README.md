...
```typescript
import { getM2MToken } from "@repo/app-clerk-m2m";
...
```typescript
import { isM2MConfigured } from "@repo/app-clerk-m2m";
...
```typescript
import { getExpectedMachineId } from "@repo/app-clerk-m2m";
...
- **@repo/app-trpc** - Creates authenticated callers for webhook and Inngest
- **@api/app** - Verifies M2M tokens in tRPC context
...
```bash
# Build
pnpm --filter @repo/app-clerk-m2m build

# Typecheck
pnpm --filter @repo/app-clerk-m2m typecheck

# Clean
pnpm --filter @repo/app-clerk-m2m clean
```
