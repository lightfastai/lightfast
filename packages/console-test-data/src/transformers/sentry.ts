/**
 * Mock Sentry Transformer for Test Data
 *
 ...
 * @see thoughts/shared/research/2026-01-22-sentry-ingestion-retrieval-pipeline.md
 */
...
export function transformSentryIssue(
  ...
  const event: SourceEvent = {
    ...
    title: `[${actionTitles[payload.action]}] ${errorType}: ${errorValue.slice(0, 80)}`,
    body: bodyParts.join("\n"),
    actor: {
      id: payload.actor.id,
...
export function transformSentryError(
  ...
  const event_out: SourceEvent = {
    ...
    title: `[Error] ${errorType}: ${errorValue.slice(0, 80)}`,
    body: bodyParts.join("\n"),
    actor: event.user
      ? {
...
export function transformSentryEventAlert(
  ...
  const event_out: SourceEvent = {
    ...
    title: `[Alert Triggered] ${triggered_rule}: ${errorType}`,
    body: bodyParts.join("\n"),
    actor: undefined,
    occurredAt: event.timestamp,
    references: refs,
...
export function transformSentryMetricAlert(
  ...
  const event: SourceEvent = {
    ...
    title: `[${actionTitle}] ${alertRule.name}`,
    body: bodyParts.join("\n"),
    actor: undefined,
