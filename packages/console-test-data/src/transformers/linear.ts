/**
 * Mock Linear Transformer for Test Data
 *
 ...
 * @see thoughts/shared/research/2026-01-22-linear-integration-ingestion-retrieval-pipeline.md
 */
...
/**
 * Linear webhook event types supported by our mock transformers.
...
export function transformLinearIssue(
  ...
  const event: SourceEvent = {
    ...
    title: `[${actionTitles[payload.action]}] ${issue.identifier}: ${issue.title.slice(0, 80)}`,
    body: bodyParts.join("\n"),
    actor: issue.creator
      ? {
          id: issue.creator.id,
...
export function transformLinearComment(
  ...
  const event: SourceEvent = {
    ...
    title: `[${actionTitles[payload.action]}] ${comment.issue.identifier}: ${comment.body.slice(0, 60)}...`,
    body: bodyParts.join("\n"),
    actor: comment.user
      ? {
          id: comment.user.id,
...
export function transformLinearProject(
  ...
  const event: SourceEvent = {
    ...
    title: `[${actionTitles[payload.action]}] Project: ${project.name}`,
    body: bodyParts.join("\n"),
    actor: project.lead
      ? {
          id: project.lead.id,
...
export function transformLinearCycle(
  ...
  const event: SourceEvent = {
    ...
    title: `[${actionTitles[payload.action]}] ${cycleName} (${cycle.team.name})`,
    body: bodyParts.join("\n"),
    actor: undefined,
    occurredAt: payload.createdAt,
    references: refs,
...
export function transformLinearProjectUpdate(
  ...
  const event: SourceEvent = {
    ...
    title: `[${actionTitles[payload.action]}] ${update.project.name}: ${update.body.slice(0, 60)}...`,
    body: bodyParts.join("\n"),
    actor: update.user
      ? {
          id: update.user.id,
