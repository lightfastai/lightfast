...
```bash
pnpm add @repo/app-reserved-names
```
...
```typescript
import { workspace } from '@repo/app-reserved-names';
...
```bash
# packages/app-reserved-names/data/organization-names.json
["_next", "_vercel", "about", "account", "admin", ..., "www"]

# packages/app-reserved-names/data/workspace-names.json
["300", "301", ..., "wiki", "www"]
```

Then rebuild:
```bash
pnpm --filter @repo/app-reserved-names build
```
...
```json
// From apps/app/microfrontends.json
{
  "lightfast-www": {
...
- `@repo/app-validation` - Zod schemas that use these reserved names
