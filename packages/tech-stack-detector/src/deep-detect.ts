...
function mergeDetectedTools(allResults: DetectionResult[]): DetectedTool[] {
  const toolMap = new Map<
    string,
    { signals: RuleMatch[]; name: string; category: DetectedTool["category"] }
  >();
...
export async function deepDetect(
  ...
  if (deep && discovered.length > 0) {
    ...
    await runConcurrent(toScan, MAX_CONCURRENT_SCANS, async (item) => {
      try {
        ...
        console.error(
          `[deep] failed to scan ${item.url}: ${(err as Error).message}`
        );
      }
    });
  }
  ...
  if (primary.unmatchedDomains) {
    for (const d of primary.unmatchedDomains) {
      allUnmatched.add(d);
    }
  }
  for (const sub of subResults) {
    if (sub.unmatchedDomains) {
      for (const d of sub.unmatchedDomains) {
        allUnmatched.add(d);
      }
    }
  }
  ...
  return {
    ...
    ...(allUnmatched.size > 0 && {
      unmatchedDomains: [...allUnmatched].sort(),
    }),
  };
}
