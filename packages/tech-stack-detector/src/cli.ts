...
function formatDiscoveredTable(discovered: DiscoveredUrl[]): string {
  ...
  if (discovered.length === 0) {
    ...
    for (const d of discovered) {
      ...
      lines.push(
        `    ${scanned} ${status} ${url} ${d.kind.padEnd(10)} (${sources})`
      );
    }
  }
...
function parseMaxScans(args: string[]): number | undefined {
  ...
  const val = Number.parseInt(String(args[idx + 1]), 10);
...
async function main() {
  ...
  const positional = args.filter(
    (a, i) => !a.startsWith("--") && args[i - 1] !== "--max-scans"
  );
  ...
  if (discoverOnly) {
    ...
    if (jsonOutput) {
      console.log(
        JSON.stringify(
          { url: normalizedUrl, domain, rootDomain, discovered },
          null,
          2
        )
      );
