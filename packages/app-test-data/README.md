...
pnpm --filter @repo/app-test-data inject -- -w <workspace_id>

# Inject a specific scenario
pnpm --filter @repo/app-test-data inject -- -w <workspace_id> -s sandbox-2

# Inject balanced mix
pnpm --filter @repo/app-test-data inject -- -w <workspace_id> -s balanced -c 10

# Stress test (100 events)
pnpm --filter @repo/app-test-data inject -- -w <workspace_id> -s stress -c 100

# Reset demo environment and inject fresh data
pnpm --filter @repo/app-test-data reset-demo -- -w <workspace_id> -i

# Verify all datasets load correctly
pnpm --filter @repo/app-test-data verify

# Verify a specific dataset
pnpm --filter @repo/app-test-data verify sandbox-1
```
...
**GitHub** (production transformers from `@repo/app-webhooks`):
...
**Vercel** (production transformer from `@repo/app-webhooks`):
...
- **GitHub/Vercel**: Production transformers from `@repo/app-webhooks`
