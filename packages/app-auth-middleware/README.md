...
{
  "dependencies": {
    "@repo/app-auth-middleware": "workspace:*"
  }
}
```
...
```typescript
import { verifyWorkspaceAccess, resolveWorkspaceBySlug } from "@repo/app-auth-middleware";
...
```typescript
import { resolveWorkspaceBySlug, resolveWorkspaceByName } from "@repo/app-auth-middleware";
...
```typescript
import { verifyResourceOwnership } from "@repo/app-auth-middleware";
...
- `@db/app` - Database schemas and client
- `@vendor/clerk` - Clerk authentication utilities
- `@repo/app-api-key` - API key crypto utilities

## Contributing

This package implements security patterns from `apps/app/SECURITY-REVIEW-2025-01-21.md`.
