...
import {
  ...
  workspaceNameSchema,
  nanoidSchema,
} from "@repo/app-validation/primitives";
...
} from "@repo/app-validation/forms";
...
} from "@repo/app-validation/database";
...
└── constants/       # Re-exported from @db/app (naming rules)
```
...
**After:**
```typescript
import { workspaceNameSchema } from "@repo/app-validation/primitives";
```
...
- `@db/app` - Database schema definitions (Drizzle)
- `@repo/app-webhooks` - Webhook verification
- `@repo/app-oauth` - OAuth state management
- `@repo/app-auth-middleware` - Authorization helpers
