...
const { BASEHUB_TOKEN, BASEHUB_CHANGELOG_TOKEN } = basehubEnv;

const basehub = basehubClient({ token: BASEHUB_TOKEN });
const basehubChangelog = basehubClient({ token: BASEHUB_CHANGELOG_TOKEN ?? BASEHUB_TOKEN });
...
export const legal = {
  ...
  getPosts: async (): Promise<LegalPost[]> => {
    const data = await basehubChangelog.query(legal.postsQuery);

    return data.legalPages.items;
  },

  getLatestPost: async (): Promise<LegalPost | null> => {
    const data = await basehubChangelog.query(legal.latestPostQuery);
  ...
  getPost: async (slug: string): Promise<LegalPost | null> => {
    ...
    const data = await basehubChangelog.query(query);
