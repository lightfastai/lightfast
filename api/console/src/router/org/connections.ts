  ...
  github: {
    ...
    list: orgScopedProcedure.query(async ({ ctx }) => {
      ...
      const allInstallations = results
        ...
        .map((row) => {
          const info = row.providerAccountInfo as Extract<
            typeof row.providerAccountInfo,
            { sourceType: "github" }
          >;
    ...
    validate: orgScopedProcedure.mutation(async ({ ctx }) => {
      ...
      try {
        ...
        const githubInstallation = await getAppInstallation(
          app,
  ...
  linear: {
    ...
    get: orgScopedProcedure.query(async ({ ctx }) => {
      ...
      return result.map((installation) => {
        ...
        const orgName =
          info?.sourceType === "linear" ? info.organization?.name : undefined;
        const orgUrlKey =
          info?.sourceType === "linear" ? info.organization?.urlKey : undefined;
