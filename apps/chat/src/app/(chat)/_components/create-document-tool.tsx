...
export const CreateDocumentTool = memo(function CreateDocumentTool({
  ...
  switch (toolPart.state) {
    ...
    case "input-available": {
      ...
      const labelBase = isPreparing ? "Preparing document creation" : "Creating document";
      const label = documentTitle ? `${labelBase}: "${documentTitle}"` : `${labelBase}...`;
              ...
              <AccordionTrigger className="items-center px-4 py-3 hover:no-underline data-[state=closed]:hover:bg-muted/50">
                <div className="flex flex-1 items-center gap-2">
                  ...
                  <div className="flex-1 text-left">
                    <div className="text-sm font-medium text-destructive">Create document failed</div>
                    {documentTitle ? (
                      <p className="mt-1 text-xs text-muted-foreground/70">Title: "{documentTitle}"</p>
                    ) : null}
                  </div>
                </div>
              </AccordionTrigger>
    ...
    case "output-available": {
      ...
      const cardBaseClasses = "flex w-full min-h-[4.5rem] items-stretch transition-colors";
        ...
        <div className="my-6 w-full overflow-hidden rounded-lg border">
          ...
          >
            <div className="flex-1 px-4 py-3 text-left">
              ...
              {documentKind ? (
                <div className="text-xs capitalize text-muted-foreground">{documentKind}</div>
              ) : null}
            </div>
