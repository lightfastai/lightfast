							...
							{events.length === 0 ? (
								...
								events.map((event, index) => (
									<div key={`${event.id}-${index}`} className="flex gap-3 p-3 rounded-lg border">
										<div className={`flex-shrink-0 ${getEventColor(event.type)}`}>{getEventIcon(event.type)}</div>
										<div className="flex-1 min-w-0">
											...
											<div className="text-sm">
												{typeof event.content === "object" ? JSON.stringify(event.content, null, 2) : event.content}
											</div>
								...
								<div className="p-4 rounded-lg bg-orange-50 border border-orange-200">
									...
									<div className="space-y-2">
										...
										<div className="font-mono text-sm bg-background p-3 rounded border">
											...
											{isRunningTest && events.some((e) => e.type === "thinking") && (
										...
										<div className="text-xs text-muted-foreground">
											Thinking tokens: {events.filter((e) => e.type === "thinking").length} • Live decision-making in
											progress
										</div>
									</div>
								</div>
							...
							{events.filter((e) => e.type === "chunk" && e.content && !e.content.includes("tool")).length > 0 && (
								<div className="p-4 rounded-lg bg-primary/5 border">
									...
									<div className="prose prose-sm space-y-2">
										...
										{/* Show response stats */}
										<div className="text-xs text-muted-foreground">Response complete</div>
									</div>
								</div>
							)}
										...
										.map((event, index) => (
											<div key={index} className="p-3 rounded-lg bg-muted text-sm">
												<div className="flex items-center gap-2 mb-1">
													...
													<span className="font-medium">{event.metadata?.tool || "Unknown Tool"}</span>
												</div>
