...
import { GitHubSourceItem } from "~/app/(app)/(org)/[slug]/[workspaceName]/(manage)/sources/new/_components/github-source-item";
...
vi.mock(
	"~/app/(app)/(org)/[slug]/[workspaceName]/(manage)/sources/new/_components/source-selection-provider",
	() => ({
		useSourceSelection: () => mockSourceSelection,
	}),
);
...
beforeEach(() => {
	vi.clearAllMocks();
	// Reset source selection to connected state
	mockSourceSelection.gwInstallationId = "gw-inst-1";
	mockSourceSelection.installations = [mockInstallation];
	mockSourceSelection.selectedInstallation = mockInstallation;
	mockSourceSelection.selectedRepositories = [];
});
...
describe("GitHubSourceItem", () => {
	...
	describe("disconnected state rendering", () => {
		beforeEach(() => {
			mockSourceSelection.gwInstallationId = null;
			mockSourceSelection.installations = [];
			mockSourceSelection.selectedInstallation = null;
		});
	...
	describe("repository selection", () => {
		it("calls toggleRepository with the clicked repo", async () => {
			...
			expect(
				mockSourceSelection.toggleRepository,
			).toHaveBeenCalledWith(MOCK_REPO);
		});

		it("shows '1 selected' badge when a repository is selected", () => {
			mockSourceSelection.selectedRepositories = [MOCK_REPO];
			renderComponent();
