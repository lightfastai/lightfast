...
function FrameworkIcon({
	framework,
	className,
}: { framework: string | null; className?: string }) {
...
export function VercelSourceItem() {
	...
	useEffect(() => {
		const prevIds = prevInstallationsRef.current
			.map((i) => i.id)
			.join(",");
						...
						{selectedProject && !showPicker ? (
							...
							<>
								<div className="flex gap-4">
									<Select
										value={
											selectedVercelInstallation?.id
										}
										onValueChange={(id) => {
											const inst =
												connectionInstallations.find(
													(i) => i.id === id,
												);
											if (inst) {
												setSelectedVercelInstallation(
									...
									{projectsError ? (
										<div className="flex flex-col items-center py-6 text-center gap-3">
											<p className="text-sm text-destructive">
												Failed to load projects. The
												connection may need to be
												refreshed.
											</p>
											<Button
