...
async function handlePullRequestEvent(payload: PullRequestPayload) {
  ...
  console.log(`[Webhook] PR ${action}: ${repository.full_name}#${pull_request.number}`);
  ...
  if (!repos[0]) {
    console.log(`[Webhook] Repository ${repository.id} not connected, skipping`);
    return;
  }
  ...
  console.log(`[Webhook] Updating ${reviews.length} review(s) for PR ${pull_request.id}`);
  ...
  console.log(`[Webhook] Successfully updated metadata for PR ${pull_request.id}`);
}
...
async function handleInstallationRepositoriesEvent(payload: {
  ...
  console.log(`[Webhook] Repositories removed from installation ${payload.installation.id}`);
    ...
    await db
      ...
      .where(
        and(
          ...
          eq(DeusConnectedRepository.githubInstallationId, payload.installation.id.toString())
        )
      );
    ...
    switch (event) {
      ...
      case "installation":
        if (body.action === "deleted") {
          console.log(`[Webhook] Installation ${body.installation.id} deleted`);
      ...
      case "repository":
        if (body.action === "deleted") {
          console.log(`[Webhook] Repository ${body.repository.id} deleted`);
          ...
          await db
            ...
            .where(
              eq(DeusConnectedRepository.githubRepoId, body.repository.id.toString())
            );
        } else if (body.action === "renamed") {
          console.log(`[Webhook] Repository renamed to ${body.repository.full_name}`);
