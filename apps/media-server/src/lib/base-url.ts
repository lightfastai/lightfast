...
const createEnvironmentUrl = (suffix: UrlSuffix = ""): string => {
  if (env.FORCE_BASE_URL) {
    return env.FORCE_BASE_URL;
  }
...
export const createImageSuccessWebhookUrl = ({
  ...
  return `${createBaseUrl()}/api/resource/create/image/success?id=${id}`;
};
...
export const createVideoSuccessWebhookUrl = ({
  ...
  return `${createBaseUrl()}/api/resource/create/video/success?id=${id}`;
};
