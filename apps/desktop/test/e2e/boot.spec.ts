...
test("boots, renderer paints, quits cleanly", async () => {
  ...
  // offline CI without an API mesh, backend calls can log connection errors on
  // mount, which is environmental, not a renderer bug. pageerror catches truly
  // broken renderer states (failed imports, throwing components, hydration
  // errors).
