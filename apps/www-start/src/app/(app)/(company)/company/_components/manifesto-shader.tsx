...
export function ManifestoShader() {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const shaderMount = shaderRef.current?.paperShaderMount;
      if (shaderMount) {
        shaderMount.setUniforms({ u_amplitude: cur.amplitude });
        shaderMount.setSpeed(cur.speed);
      }
