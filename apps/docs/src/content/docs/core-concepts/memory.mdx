...
```typescript
import { RedisMemory } from "@lightfastai/core/agent/memory/adapters/redis";
...
```typescript
import { InMemoryMemory } from "@lightfastai/core/memory/adapters/in-memory";
...
```typescript
import { Memory } from "@lightfastai/core/memory";
