...
For rapid prototyping when you're running `bun dev:all`:
...
# 3. Update context
echo "✅ Completed theme context" >> "$CONTEXT_FILE"
...
```bash
# Progress updates
gh pr comment $PR_NUM --body "🔧 Progress: Added theme context and toggle component"

# Blockers
gh pr comment $PR_NUM --body "⚠️ Blocker: TypeScript error in theme hook"

# Completion
gh pr comment $PR_NUM --body "✅ Completed: Dark mode fully implemented"
```
...
```bash
# Debug build issues
SKIP_ENV_VALIDATION=true bun run build --debug
