...
const nextAppRoot = resolve(repoRoot, "apps/app");
const tanstackAppRoot = resolve(repoRoot, "apps/app-tanstack");
...
describe("app-tanstack migration readiness", () => {
  ...
  it("keeps the TanStack app deployable under the old app environment surface", () => {
    const packageJson = JSON.parse(
      readWorkspaceFile("apps/app-tanstack/package.json")
    ) as {
      dependencies?: Record<string, string>;
      devDependencies?: Record<string, string>;
      scripts?: Record<string, string>;
    };
    const envSource = readWorkspaceFile("apps/app-tanstack/src/env.ts");
    const viteConfigSource = readWorkspaceFile(
      "apps/app-tanstack/vite.config.ts"
    );
    const vercelConfig = JSON.parse(
      readWorkspaceFile("apps/app-tanstack/vercel.json")
    ) as { framework?: string };
