...
if (isDev && !isBuildPhase && canonicalAppOrigin === productionAppOrigin) {
  throw new Error(
    "[cors] app-tanstack app URL resolved to production URL in dev; portless env injection is missing. " +
      "Run `pnpm dev` from the workspace root before starting app-tanstack."
  );
}
