...
const appNextRoot = resolve(repoRoot, "apps/app-next");
...
describe("microfrontends config", () => {
  ...
  it("pins deprecated Next microfrontends config loading to the app mesh", () => {
    const appNextConfig = readFileSync(
      resolve(appNextRoot, "next.config.ts"),
      "utf8"
    );
