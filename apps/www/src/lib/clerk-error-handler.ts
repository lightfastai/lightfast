...
export function handleClerkError(
  ...
  if (!errorData || typeof errorData !== "object") {
    ...
    if (errorData instanceof Error) {
      ...
      if (
        errorData.message.includes("fetch failed") ||
    ...
    if (Array.isArray(response.errors) && response.errors.length > 0) {
      ...
      if (response.errors.length > 1) {
        console.warn(
          "[Clerk] Multiple errors received, handling first:",
          response.errors
        );
      }
    }
  ...
  if (code) {
    switch (code) {
      ...
      default:
        ...
        if (
          code &&
          (code.startsWith("form_param") || code.includes("_invalid"))
        ) {
          isValidationError = true;
          shouldLog = false;
        }
  ...
  if (shouldLog) {
    ...
    captureException(sentryError, {
      ...
      extra: {
        ...
        errorCount: Array.isArray((errorData as Record<string, unknown>).errors)
          ? ((errorData as Record<string, unknown>).errors as unknown[]).length
          : undefined,
      },
