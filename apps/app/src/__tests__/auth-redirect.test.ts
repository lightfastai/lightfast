...
import { oauthRequestRedirectTarget } from "~/oauth/oauth-auth-redirect";
...
describe("oauthRequestRedirectTarget", () => {
  it("normalizes absolute app request URLs to relative redirect targets", () => {
    expect(
      oauthRequestRedirectTarget(
        "https://app.lightfast.localhost/oauth/authorize?client_id=cli#consent"
      )
    ).toBe("/oauth/authorize?client_id=cli#consent");
  });

  it("preserves relative request paths and falls back for malformed values", () => {
    expect(oauthRequestRedirectTarget("/oauth/cli/start?state=abc")).toBe(
      "/oauth/cli/start?state=abc"
    );
    expect(oauthRequestRedirectTarget("not a url")).toBe("/");
  });
});
