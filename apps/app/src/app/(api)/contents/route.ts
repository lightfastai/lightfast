...
import { ContentsRequestSchema } from "@repo/console-validation";
...
export async function POST(request: NextRequest) {
  ...
  log.info("Contents API request", { requestId });
  ...
  try {
    ...
    const parsed = ContentsRequestSchema.safeParse(body);
    if (!parsed.success) {
      return NextResponse.json(
        {
    ...
    log.error("Contents API handler error", {
