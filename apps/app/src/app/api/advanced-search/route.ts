...
// Set up periodic cache cleanup if we're in a server environment
if (typeof window === "undefined") {
  ...
  // For development mode, clean up interval on hot reload
  if (process.env.NODE_ENV === "development") {
    // We're using a function that simulates module dispose
    // This is safer than directly accessing module.hot which causes type errors
    const cleanup = () => {
      clearInterval(interval);
    };

    // Store cleanup function in global scope for HMR to access if needed
    // @ts-ignore
    if (global.__CLEANUP_FUNCTIONS__) {
      // @ts-ignore
      global.__CLEANUP_FUNCTIONS__.push(cleanup);
    } else {
      // @ts-ignore
      global.__CLEANUP_FUNCTIONS__ = [cleanup];
    }
  }
}
