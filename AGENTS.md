...
describe("TaskAnalyzerAgent", () => {
	...
	it("should analyze a simple task correctly", async () => {
		const input = "Create a simple calculator";
		...
		expect(result.object.complexity).toBeOneOf(["low", "medium", "high"]);
		expect(result.object.requirements).toBeInstanceOf(Array);
		expect(result.object.requirements.length).toBeGreaterThan(0);
	});

	it("should handle complex tasks", async () => {
		const input = "Build a distributed microservices architecture with authentication";
		...
		expect(result.object.complexity).toBe("high");
	});
});
```
...
describe("SandboxExecutorTool", () => {
	it("should execute safe commands successfully", async () => {
		...
		const result = await sandboxExecutorTool.execute({
			context: input,
	...
	it("should reject unsafe commands", async () => {
		...
		expect(result.stderr).toContain("Unsafe command detected");
	});
});
```
...
describe("TaskExecutionWorkflow", () => {
	...
	beforeEach(() => {
		mastra = new Mastra({
			agents: { taskAnalyzer: taskAnalyzerAgent },
	...
	it("should execute a complete task workflow", async () => {
		...
		const run = await mastra.getWorkflow("taskExecution").createRunAsync();
		const result = await run.start({ inputData: input });

		expect(result.status).toBe("success");
		expect(result.result?.success).toBe(true);
		expect(result.result?.steps).toBeInstanceOf(Array);
		expect(result.result?.result).toContain("Successfully executed");
	});

	it("should handle workflow suspension and resumption", async () => {
		const input = { request: "Approve deployment to production" };

		const run = await mastra.getWorkflow("approvalWorkflow").createRunAsync();
		const result = await run.start({ inputData: input });
		...
		expect(resumedResult.result?.approved).toBe(true);
	});
});
```
