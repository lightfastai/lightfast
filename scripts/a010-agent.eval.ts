...
function extractToolCalls(result: any): any[] {
	...
	const toolPatterns = [/using (.+?) tool/gi, /executing (.+?) command/gi, /calling (.+?) function/gi];
...
// Type definitions for the evaluation
type A010Input = {
	query: string;
	category: string;
	complexity: string;
...
// Main evaluation
Eval<A010Input, string, void, A010Metadata>("a010-comprehensive-evaluation", {
	data: a010TestScenarios,

	task: async (input) => {
		const result = await executeA010Agent(input);
		...
		await logAgentInteraction(
			...
			await scoreA010Performance(input, result.output, result.toolCalls, result.duration, result.error),
			{} as A010Metadata, // Metadata comes separately in scorer
		);
	...
	scores: [
		async (evalCase: any, output: string) => {
			const input = evalCase.input as A010Input;
			// For Braintrust UI, we need to return the scores
			const result = await executeA010Agent(input);
			const scores = await scoreA010Performance(input, output, result.toolCalls, result.duration, result.error);

			console.log(`[EVAL] Scores for ${input.category}:`, scores);
