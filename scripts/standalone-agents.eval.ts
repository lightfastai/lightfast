...
async function executeStandaloneAgent(scenario: StandaloneAgentScenario): Promise<{
	output: string;
	duration: number;
	error?: string;
	toolsUsed?: string[];
	...
	try {
		const agent = mastra.getAgent(scenario.input.agent as any);
...
async function scoreStandaloneAgent(
	scenario: StandaloneAgentScenario,
	output: string,
	duration: number,
	toolsUsed: string[] = [],
	error?: string,
...
// Main evaluation
Eval<StandaloneAgentScenario["input"], string, void, StandaloneAgentScenario["metadata"]>(
	"standalone-agents-evaluation",
	{
		data: standaloneAgentScenarios,

		task: async (input) => {
			const result = await executeStandaloneAgent({ input, metadata: {} as any });
		...
		scores: [
			async (evalCase: any, output: string) => {
				const { input, metadata } = evalCase as StandaloneAgentScenario;
				const result = await executeStandaloneAgent({ input, metadata });
				...
				console.log(
					`[EVAL] ${input.agent} - ${input.category}:`,
