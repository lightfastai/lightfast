...
async function simulateAgentResponse(agentName: string, scenario: any): Promise<string> {
...
function scoreExperimentalAgent(scenario: any, output: string): Record<string, number> {
	...
	if (scenario.agent === "v010") {
		...
		scores.multi_tool_coordination = (outputLower.match(/\b(search|file|sandbox|browser)\b/g) || []).length >= 2 ? 1 : 0.5;
	}
	
	if (scenario.agent === "v011") {
...
Eval("hal9000-experimental-agents", {
	...
	task: async (scenario) => {
		...
		// Braintrust passes the input object directly
		const agentName = scenario.agent;
		const category = scenario.category;
		const complexity = scenario.complexity;
		
		console.log(`[EVAL] Testing ${agentName} agent - ${category} (${complexity} complexity)`);
		
		const output = await simulateAgentResponse(agentName, scenario);
	...
	scores: [
		(scenario, output) => {
			const agentName = scenario.agent;
			const category = scenario.category;
			const scores = scoreExperimentalAgent(scenario, output);
			console.log(`[EVAL] Scores for ${agentName} (${category}):`, 
...
console.log(`
🧪 HAL9000 Experimental Agent Evaluation
