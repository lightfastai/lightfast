...
async function runLocalEvaluation(scenario: TestScenario) {
	console.log(`\n${"=".repeat(60)}`);
	...
	try {
		...
		// Create messages
		const messages: CoreMessage[] = [{ role: "user", content: scenario.query }];
...
function evaluateAccuracy(response: string, scenario: TestScenario): number {
