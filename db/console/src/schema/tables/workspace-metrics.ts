...
import type { MetricType, MetricUnit } from "@repo/console-validation";
...
export const workspaceMetrics = pgTable(
  "lightfast_workspace_metrics",
  {
     ...
     */
    type: varchar("type", { length: 50 }).notNull().$type<MetricType>(),
     ...
     */
    unit: varchar("unit", { length: 20 }).$type<MetricUnit>(),
  ...
  (table) => ({
    ...
    workspaceTypeTimestampIdx: index("metric_workspace_type_timestamp_idx").on(
      table.workspaceId,
      table.type,
      table.timestamp,
    ),
