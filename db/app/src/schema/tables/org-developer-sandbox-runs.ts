...
export const orgDeveloperSandboxRuns = mysqlTable(
  ...
  (table) => ({
    publicIdUq: uniqueIndex("org_developer_sandbox_runs_public_id_uq").on(
      table.publicId
    ),
    orgActorStatusIdx: index(
      "org_developer_sandbox_runs_org_actor_status_idx"
    ).on(table.clerkOrgId, table.actorUserId, table.status),
    expiresStatusIdx: index("org_developer_sandbox_runs_expires_status_idx").on(
      table.expiresAt,
      table.status
    ),
  })
...
export const orgDeveloperSandboxCommands = mysqlTable(
  ...
  (table) => ({
    publicIdUq: uniqueIndex("org_developer_sandbox_commands_public_id_uq").on(
      table.publicId
    ),
    runStatusIdx: index("org_developer_sandbox_commands_run_status_idx").on(
      table.sandboxRunId,
      table.status
    ),
