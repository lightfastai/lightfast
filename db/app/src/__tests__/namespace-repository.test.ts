...
describe("namespace repository", () => {
  ...
  it("returns an already active namespace during backfill for the same owner", async () => {
    const namespace = {
      activeOperationId: null,
      claimedClerkOrgId: null,
      claimedClerkUserId: "user_123",
    ...
    const select = vi.fn(() => ({
      from: () => ({
        where: () => ({
          limit: () => Promise.resolve([namespace]),
        }),
      }),
    }));
    const insert = vi.fn();
  ...
  it("backfills an existing Clerk org into an active namespace", async () => {
    const started = operation({
      clerkOrgId: "org_123",
      clerkUserId: null,
      idempotencyClerkOrgId: "org_123",
      idempotencyClerkUserId: null,
      operationType: "backfill_existing_handle",
      ownerKind: "org",
      status: "started",
      toHandle: "acme-inc",
    });
    const reserved = { ...started, status: "namespace_reserved" as const };
    const clerkApplied = { ...started, status: "clerk_applied" as const };
    const finalized = { ...started, status: "finalized" as const };
    ...
    const values = vi.fn((value: unknown) => {
      ...
      return { $returningId: () => Promise.resolve([{ id: 1 }]) };
    });
    const insert = vi.fn(() => ({ values }));
    const setCalls: unknown[] = [];
    const where = vi.fn(() => ({ affectedRows: 1 }));
    ...
    expect(setCalls).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ status: "namespace_reserved" }),
        expect.objectContaining({ status: "clerk_applied" }),
        expect.objectContaining({
          activeOperationId: null,
          claimedClerkOrgId: "org_123",
