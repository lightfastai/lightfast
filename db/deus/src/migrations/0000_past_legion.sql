...
--> statement-breakpoint
CREATE TABLE `lightfast_deus_api_keys` (
	`id` varchar(191) NOT NULL,
	...
	`created_at` datetime NOT NULL DEFAULT (CURRENT_TIMESTAMP),
	`updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
	CONSTRAINT `lightfast_deus_api_keys_id` PRIMARY KEY(`id`),
	CONSTRAINT `lightfast_deus_api_keys_key_hash_unique` UNIQUE(`key_hash`)
);
--> statement-breakpoint
CREATE TABLE `lightfast_deus_sessions` (
	`id` varchar(191) NOT NULL,
	`organization_id` varchar(191) NOT NULL,
	`repository_id` varchar(191),
	`user_id` varchar(191) NOT NULL,
	`status` enum('active','paused','completed') NOT NULL DEFAULT 'active',
	`current_agent` enum('deus','claude-code','codex'),
	`cwd` varchar(500) NOT NULL,
	`metadata` json,
	`created_at` datetime NOT NULL DEFAULT (CURRENT_TIMESTAMP),
	`updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
	CONSTRAINT `lightfast_deus_sessions_id` PRIMARY KEY(`id`)
);
--> statement-breakpoint
CREATE TABLE `lightfast_deus_messages` (
	`id` varchar(191) NOT NULL,
	...
	CONSTRAINT `lightfast_deus_messages_id` PRIMARY KEY(`id`)
);
--> statement-breakpoint
...
CREATE INDEX `org_clerk_org_idx` ON `lightfast_deus_organizations` (`clerk_org_id`);--> statement-breakpoint
...
CREATE INDEX `review_status_idx` ON `lightfast_deus_code_review_tasks` (`code_review_id`,`status`);--> statement-breakpoint
CREATE INDEX `key_hash_idx` ON `lightfast_deus_api_keys` (`key_hash`);--> statement-breakpoint
CREATE INDEX `org_id_idx` ON `lightfast_deus_api_keys` (`organization_id`);--> statement-breakpoint
CREATE INDEX `user_id_idx` ON `lightfast_deus_api_keys` (`user_id`);--> statement-breakpoint
CREATE INDEX `org_created_at_idx` ON `lightfast_deus_sessions` (`organization_id`,`created_at`);--> statement-breakpoint
CREATE INDEX `user_id_idx` ON `lightfast_deus_sessions` (`user_id`);--> statement-breakpoint
CREATE INDEX `repository_id_idx` ON `lightfast_deus_sessions` (`repository_id`);--> statement-breakpoint
CREATE INDEX `session_id_created_at_idx` ON `lightfast_deus_messages` (`session_id`,`created_at`);