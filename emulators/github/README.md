...
Production packages may import `@lightfast/connector-github/contract` and
`@lightfast/connector-github/node`; they must not import `@repo/github-emulator`
or files under `emulators/github`. The emulator is local infrastructure for the
same GitHub-compatible endpoints used by the production-shaped setup flow.
