...
describe("InMemoryMemory", () => {
  ...
  describe("message management", () => {
    ...
    it("should handle multiple sessions independently", async () => {
      ...
      expect(messages1[0]).toBeDefined();
      expect(getMessageText(messages1[0] ?? ({} as UIMessage))).toBe(
        "Session 1 message"
      );
      expect(messages2[0]).toBeDefined();
      expect(getMessageText(messages2[0] ?? ({} as UIMessage))).toBe(
        "Session 2 message"
      );
    });
