...
import {
  ...
  createUserMessage,
} from "../test-utils/message-helpers";
import {
  NoUserMessageError,
...
const mockCreateNewResumableStream = vi.fn(
  async (_streamId: string, getStream: () => ReadableStream<string>) => {
...
const mockResumeExistingStream = vi
  .fn()
...
describe("Runtime Functions", () => {
  ...
  describe("streamChat", () => {
    ...
    const mockStreamResult = {
      ...
      fullStream: createAsyncIterableStream(
        (async function* () {
          yield { type: "text-delta" as const, id: "delta1", text: "test" };
        })()
      ),
      reasoningStream: createAsyncIterableStream(
        (async function* () {
          yield "reasoning";
        })()
      ),
      ...
      reasoning: Promise.resolve([
        { type: "reasoning" as const, text: "test reasoning" },
      ]),
      ...
      experimental_partialOutputStream: createAsyncIterableStream(
        (async function* () {
          yield {};
        })()
      ),
    ...
    beforeEach(() => {
      ...
      mockMemory.getSession = vi
        .fn()
        .mockResolvedValue({ resourceId: "resource1" });
      mockMemory.appendMessage = vi.fn().mockResolvedValue(undefined);
    ...
    it("should handle session validation failures", async () => {
      mockMemory.getSession = vi
        .fn()
        .mockResolvedValue({ resourceId: "different-resource" });
  ...
  describe("Guard-Based Failure Handling", () => {
    ...
    it("should fail fast with failOnStreamError guard", async () => {
      ...
      mockMemory.createStream = vi
        .fn()
        .mockRejectedValue(new Error("Critical stream error"));
  ...
  describe("Memory Failure Edge Cases During Streaming", () => {
    ...
    it("should provide detailed error context for memory failures", async () => {
      ...
      expect(errorCall?.[0].systemContext).toEqual({
        sessionId: "session1",
        resourceId: "resource1",
      });
    });
