...
import type { LightfastErrorSource } from "./error-classification";
import {
...
>(
  ...
  const handleStreamError = (error: unknown) =>
    streamErrorToPayload(error, true);
  const writerStreamError = (error: unknown) =>
    streamErrorToPayload(error, false);
  ...
  if (
    shouldEnableResume &&
    resumeOptions?.failOnStreamError &&
    resumeSetupPromise
  ) {
    try {
