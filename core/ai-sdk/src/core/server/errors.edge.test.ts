...
describe("Error Handling - Critical Edge Cases", () => {
  describe("Error Inheritance and Serialization", () => {
    ...
    it("should handle AgentStreamError with cause chain", () => {
      ...
      expect(streamError.message).toBe(
        "Agent streaming failed: Model configuration invalid"
      );
  ...
  describe("Error Message Sanitization", () => {
    ...
    it("should handle errors with very long messages", () => {
      const longMessage = "A".repeat(10_000); // 10KB message
  ...
  describe("toAgentApiError Converter Function", () => {
    it("should pass through existing ApiError instances unchanged", () => {
      const existingError = new AgentConfigurationError(
        "model",
        "Missing model"
      );
    ...
    it("should convert model configuration errors", () => {
      ...
      expect(result.message).toBe(
        "Agent configuration error: model - Model configuration is invalid"
      );
    ...
    it("should convert context creation errors", () => {
      ...
      expect(result.message).toBe(
        "Failed to create runtime context: Context creation failed"
      );
      expect(result.statusCode).toBe(400);
    });
    ...
    it("should convert tool factory errors", () => {
      ...
      expect(result.message).toBe(
        "Tool execution failed: unknown - Tool factory execution failed"
      );
    ...
    it("should convert cache operation errors", () => {
      ...
      expect(result.message).toBe(
        "Cache operation failed: caching - Cache operation failed"
      );
      expect(result.statusCode).toBe(500);
    });
    ...
    it("should convert message conversion errors", () => {
      ...
      expect(result.message).toBe(
        "Message conversion failed: convertMessages - Message conversion failed"
      );
      expect(result.statusCode).toBe(400);
    });
    ...
    it("should use operation-specific mapping for stream operations", () => {
      ...
      expect(result.message).toBe(
        "Agent streaming failed: Unknown stream error"
      );
      expect(result.statusCode).toBe(500);
    });
    ...
    it("should use operation-specific mapping for createRuntimeContext", () => {
      ...
      expect(result).toBeInstanceOf(ContextCreationError);
      expect(result.message).toBe(
        "Failed to create runtime context: Unknown context error"
      );
      expect(result.statusCode).toBe(400);
    });
    ...
    it("should use operation-specific mapping for resolveTools", () => {
      ...
      expect(result).toBeInstanceOf(ToolExecutionError);
      expect(result.message).toBe(
        "Tool execution failed: factory - Unknown tool error"
      );
      expect(result.statusCode).toBe(500);
    });
    ...
    it("should handle unknown operations with default mapping", () => {
      ...
      expect(result).toBeInstanceOf(AgentStreamError);
      expect(result.message).toBe(
        "Agent streaming failed: unknownOperation: Unknown operation error"
      );
      expect(result.statusCode).toBe(500);
    });
    ...
    it("should handle non-Error thrown values", () => {
      ...
      expect(result1.message).toBe(
        "Agent streaming failed: stream: string error"
      );
      ...
      expect(result3.message).toBe(
        "Agent streaming failed: stream: [object Object]"
      );
    });
