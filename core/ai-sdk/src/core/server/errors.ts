...
import type { LightfastErrorContext } from "./error-classification";
import {
...
export class AgentConfigurationError extends BadRequestError {
  ...
  constructor(field: string, details?: string, options: ApiErrorOptions = {}) {
    super(
      `Agent configuration error: ${field}${details ? ` - ${details}` : ""}`,
      {
        ...options,
...
export class ContextCreationError extends BadRequestError {
  ...
  constructor(
    ...
    super(
      `Failed to create ${contextType} context${message ? `: ${message}` : ""}`,
      {
        ...options,
        cause,
        category: options.category ?? LightfastErrorCategory.Infrastructure,
...
export class CacheOperationError extends ApiError {
  ...
  constructor(
    ...
    super(
      `Cache operation failed: ${operation}${message ? ` - ${message}` : ""}`,
      {
        ...options,
        cause,
...
export class MessageConversionError extends BadRequestError {
  ...
  constructor(
    ...
    super(
      `Message conversion failed: ${operation}${message ? ` - ${message}` : ""}`,
      {
        ...options,
        cause,
...
function mapAiSdkErrorToApiError(
  error: AISDKError,
  operation: string
): ApiError {
...
export function toAgentApiError(error: unknown, operation: string): ApiError {
  ...
  if (error instanceof Error) {
    ...
    if (
      ...
      (message.includes("factory") || message.includes("execution"))
    ) {
    ...
    if (
      ...
      message.includes("messages required")
    ) {
