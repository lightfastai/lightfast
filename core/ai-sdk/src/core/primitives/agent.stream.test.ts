...
describe("Agent buildStreamParams - Critical Edge Cases", () => {
  ...
  describe("Tool Configuration Edge Cases", () => {
    it("should configure tools that modify shared state", () => {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        expect(typeof streamParams.tools?.statefulTool?.execute).toBe(
          "function"
        );
      }
    });
    ...
    it("should configure tools with potential circular dependencies", () => {
      ...
      const streamParams = agent.buildStreamParams({
        ...
        messages: [createUserMessage("1", "Start circular tool usage")],
        memory,
        resourceId: "user-123",
        systemContext: {
          sessionId: "test-session",
    ...
    it("should configure tools with strict schema validation", () => {
      ...
      const invalidInputs = [
        ...
        { options: new Array(10).fill("opt") }, // too many options
      ];
      ...
      for (const [index, _invalidInput] of invalidInputs.entries()) {
        ...
        const streamParams = agent.buildStreamParams({
          ...
          messages: [createUserMessage("1", "Use tool with invalid input")],
          memory,
          resourceId: "user-123",
          systemContext: {
            sessionId: "test-session",
  ...
  describe("Memory and Resource Edge Cases", () => {
    it("should build params with large conversation histories", async () => {
      ...
      const largeMessages: UIMessage[] = Array.from({ length: 100 }, (_, i) =>
        i % 2 === 0
