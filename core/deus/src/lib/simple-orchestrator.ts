...
export class SimpleOrchestrator {
  ...
  async initialize(): Promise<void> {
    ...
    console.log(`[SimpleOrchestrator] Initialized with session: ${this.state.sessionId}`);
  }
    ...
    try {
      ...
      if (action.config.mcpServers.length > 0) {
        ...
        if (process.env.DEBUG) {
          console.log('[SimpleOrchestrator] Starting agent without MCP configuration');
        }
      }
    ...
    if (prompt && prompt.trim()) {
      setTimeout(async () => {
        try {
          ...
          console.error('[SimpleOrchestrator] Error sending initial prompt:', error);
        }
  ...
  private async startCodex(prompt: string): Promise<void> {
    ...
    if (process.env.DEBUG && !this.mcpOrchestrator) {
      console.log('[SimpleOrchestrator] Starting Codex without MCP configuration');
    }
