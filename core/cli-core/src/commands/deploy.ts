...
export const deployCommand = new Command("deploy")
  ...
  .option("-n, --name <name>", "Deployment name (defaults to package.json name)")
  .option("--version <version>", "Deployment version (defaults to package.json version)")
...
`)
  .action(async (options: DeployOptions) => {
    ...
    try {
      ...
      const profileName = options.profile || await configStore.getDefaultProfile();
      const profile = await configStore.getProfile(profileName);
      
      if (!profile) {
        console.error(chalk.red("✖ Authentication required"));
        console.error(chalk.gray(`  Profile '${profileName}' not found`));
        console.error(chalk.gray("  Run 'lightfast auth login' to authenticate"));
        if (options.profile) {
          console.error(chalk.gray(`  Or run 'lightfast auth login --profile ${options.profile}' for this profile`));
        }
        process.exit(1);
      }
      ...
      if (!apiKey) {
        ...
        console.error(chalk.gray(`  Profile '${profileName}' exists but has no stored credentials`));
        console.error(chalk.gray("  Run 'lightfast auth login' to authenticate"));
        process.exit(1);
      }
      ...
      if (!whoamiResult.success) {
        ...
        console.error(chalk.gray(`  Error: ${whoamiResult.message || whoamiResult.error}`));
      ...
      if (options.verbose) {
        ...
        if (whoamiResult.data.organization) {
          console.log(chalk.gray(`  Organization: ${whoamiResult.data.organization.name}`));
        }
      }
      ...
      if (!existsSync(resolvedConfigPath)) {
        ...
        console.error(chalk.gray("  Create a lightfast.config.ts file to define your agents"));
        console.error(chalk.gray("  See https://docs.lightfast.ai/config for examples"));
        process.exit(1);
      }
      ...
      spinner = new CompilationSpinner("Compiling and bundling for deployment...");
      spinner.start();
      ...
      if (options.verbose) {
        console.log(chalk.gray(`  Bundles generated: ${bundleResult.bundles.length}`));
        bundleResult.bundles.forEach(bundle => {
          console.log(chalk.gray(`    - ${bundle.id} (${bundle.hash.substring(0, 8)}...) - ${bundle.size} bytes`));
        });
      }
      ...
      if (!deployResult.success) {
        ...
        console.error(chalk.red(`  Error: ${deployResult.message || deployResult.error}`));
        
        if (deployResult.error === "HTTP 400") {
          console.error(chalk.gray("  Check your deployment configuration and try again"));
        } else if (deployResult.error === "HTTP 403") {
          console.error(chalk.gray("  You may not have permission to deploy to this environment"));
      ...
      console.log(chalk.gray(`  Deployment ID: ${deployResult.data.deploymentId}`));
      console.log(chalk.gray(`  URL: ${deployResult.data.url}`));
      console.log(chalk.gray(`  Status: ${deployResult.data.status}`));
      console.log(chalk.gray(`  Environment: ${environment}`));
      
      if (deployResult.data.status === 'pending' || deployResult.data.status === 'building') {
        console.log("");
        console.log(chalk.yellow("⏳ Your deployment is being processed"));
        console.log(chalk.gray("  Check status with: lightfast deployments status <deployment-id>"));
        console.log(chalk.gray("  View logs with: lightfast deployments logs <deployment-id>"));
      }

      console.log("");
      console.log(chalk.cyan("📋 Next Steps:"));
      console.log(chalk.gray(`  • Monitor your deployment at: ${getDashboardUrl()}`));
      console.log(chalk.gray("  • View deployment status: lightfast deployments list"));
      console.log(chalk.gray("  • Test your agent: curl -X POST " + deployResult.data.url));
      ...
      if (error instanceof Error) {
        ...
        if (error.message.includes("ENOENT")) {
          console.error(chalk.gray("  File not found - check your configuration path"));
        } else if (error.message.includes("permission") || error.message.includes("EACCES")) {
          console.error(chalk.gray("  Permission denied - check file/directory permissions"));
        } else if (error.message.includes("network") || error.message.includes("fetch")) {
          console.error(chalk.gray("  Network error - check your internet connection"));
        }
      } else {
      ...
      console.error(chalk.gray("\nFor help:"));
      console.error(chalk.gray("  • Check documentation: https://docs.lightfast.ai"));
      console.error(chalk.gray("  • Verify authentication: lightfast auth status"));
