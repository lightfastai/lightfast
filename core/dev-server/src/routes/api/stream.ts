 ...
 */
export const ServerRoute = createServerFileRoute("/api/stream").methods({
	GET: async ({ request }) => {
		...
		console.info(
			`📥 GET /api/stream?agentId=${agentId}&sessionId=${sessionId}`,
		);
	...
	POST: async ({ request }) => {
		try {
			...
			if (!agent) {
					...
					return json(
						{
							...
							message:
								"Please ensure lightfast.config.ts is properly configured with at least one agent.",
						},
						{ status: 500 },
					);
				}
				...
				console.warn(
					`! Agent "${agentId}" not found. Available agents:`,
					availableAgentIds,
				);
				return json(
					{
	...
	DELETE: async ({ request }) => {
		...
		console.info(`🗑 DELETE /api/stream?sessionId=${sessionId}`);
