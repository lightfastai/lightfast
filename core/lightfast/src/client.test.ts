...
describe("LightfastMemory", () => {
  ...
  describe("constructor", () => {
    ...
    it("should accept valid apiKey", () => {
      const memory = new LightfastMemory({ apiKey: "sk_live123test" });
      expect(memory).toBeInstanceOf(LightfastMemory);
    });

    it("should use default baseUrl", () => {
      const memory = new LightfastMemory({ apiKey: "sk_test123abc" });
      expect(memory).toBeInstanceOf(LightfastMemory);
    });

    it("should accept custom baseUrl", () => {
      const memory = new LightfastMemory({
        apiKey: "sk_test123abc",
  ...
  describe("factory function", () => {
    it("should create LightfastMemory instance", () => {
      const memory = createLightfastMemory({ apiKey: "sk_test123abc" });
      expect(memory).toBeInstanceOf(LightfastMemory);
    });
  });
});
