...
#### W4 — Per-surface preload isolation scaffolding

**Gates:** wants a real second-surface consumer to justify the split. Could be triggered by:
- Adding an in-app browser/web-view surface
- Adding a third-party plugin host
- Trust boundary for the HUD window

If no consumer is pending, this group is a slot-holder; the merge cost of doing it later is identical to doing it now.
