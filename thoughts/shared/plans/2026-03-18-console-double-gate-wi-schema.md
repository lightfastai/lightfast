...
When `eventAllowed` is false (line 319), the job completes with `reason: "event_not_allowed"`. There is no `"inactive_connection"` reason code in the current schema. Adding it requires updating `eventCaptureOutputFilteredSchema` in `@repo/console-validation`.
...
- `pnpm --filter @api/console build` passes (tRPC types compile)
...
## Phase 2: Update `@repo/console-validation` — Filtered Reason Enum
...
### Success Criteria:
#### Automated Verification:
- [ ] `pnpm --filter @repo/console-validation build` exits 0
...
### Success Criteria:
#### Automated Verification:
- [ ] `pnpm --filter @db/console build` exits 0
- [ ] `pnpm typecheck` exits 0
...
- [ ] `pnpm --filter @api/console build` exits 0 (tRPC types compile)
...
### Success Criteria:
#### Automated Verification:
- [ ] `pnpm --filter @api/console build` exits 0
- [ ] `pnpm typecheck` exits 0
- [ ] `pnpm check` exits 0
...
- `pnpm --filter @api/console build` — validates tRPC output types include the new reason
